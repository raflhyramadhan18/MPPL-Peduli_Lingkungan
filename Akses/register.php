...
$error = '';
// Variabel $success di sini sudah tidak digunakan karena kita pakai Flash Message
// $success = ''; 
    ...
    if (empty($username) || empty($password) || empty($confirm_password)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if ($stmt->execute([$username, $hashed])) {
                // *** LOGIKA REDIRECT OTOMATIS DAN FLASH MESSAGE BARU ***
...
<div class="flex flex-col md:flex-row w-[900px] bg-white/10 rounded-2xl overflow-hidden shadow-2xl border border-white/20 backdrop-blur-xl">
  ...
  <div class="md:w-1/2 p-10 flex flex-col justify-center bg-white/70 backdrop-blur-xl">
    ...
    <?php 
    // BLOK PHP UNTUK MENAMPILKAN $success DIHAPUS 
    // KARENA REDIRECT SUDAH DILAKUKAN DI ATAS
    ?>
