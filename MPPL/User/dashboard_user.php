...
try {
    $stmt = $pdo->prepare("SELECT 
        COUNT(CASE WHEN status IN ('pending', 'verifikasi', 'proses', 'baru') THEN 1 END) AS total,
        COUNT(CASE WHEN status IN ('pending', 'verifikasi', 'proses', 'baru') THEN 1 END) AS pending,
    ...
    if ($stats) {
        $total_laporan = $stats['total'];
        $laporan_pending = $stats['pending'];
        $laporan_selesai = $stats['selesai'];
    }

    $stmt_terbaru = $pdo->prepare("SELECT 
        id, 
    ...
    $stmt_terbaru->execute([$username]);
    $laporan_terbaru = $stmt_terbaru->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("Database Error in dashboard_user.php: " . $e->getMessage());
}
?>
